...
-(BOOL)createDB{
    ...
    if ([filemgr fileExistsAtPath: databasePath ] == NO)
    {
        ...
        if (sqlite3_open(dbpath, &database) == SQLITE_OK)
        {
            char *errMsg;
            const char *sql_stmt =
            "create table if not exists projectsDetail (projectId integer primary key autoincrement, projectName text, resourcePath text,captionImagePath text,creationDate DATETIME)";
